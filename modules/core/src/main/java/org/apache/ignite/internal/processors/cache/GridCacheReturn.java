...
public class GridCacheReturn implements Message {
    /**
     * Delegate for {@code CacheObject} marshal traversal (generated by APT).
     *
     * FIXME IGNITE-XXXXX: remove this field together with {@link #prepareMarshal(GridCacheContext)} once the
     * communication framework invokes {@code MessageSerializer#prepareMarshalCacheObjects} for tx response classes
     * that don't extend {@code GridCacheIdMessage} or that need {@code retVal}'s own {@code cacheId} for the
     * {@code CacheObjectContext}. Until then we delegate {@code CacheObject} traversal to the generated serializer
     * to avoid duplicating logic, accepting a transient source &rarr; generated-source dependency.
     */
    private static final GridCacheReturnSerializer SER = new GridCacheReturnSerializer();
     ...
     *
     * Bridge into the generated {@link GridCacheReturnSerializer#prepareMarshalCacheObjects}.
     * Used by tx response classes that either don't extend {@code GridCacheIdMessage}
     * (so the comm-layer auto-invoke does not fire) or that need {@code retVal}'s own
     * {@code cacheId} for the {@code CacheObjectContext} (different from the outer
     * message's). See {@link GridDhtTxFinishResponse}, {@link GridNearTxPrepareResponse}.
     */
    public void prepareMarshal(GridCacheContext ctx) throws IgniteCheckedException {
        assert !loc;

        SER.prepareMarshalCacheObjects(this, ctx.cacheObjectContext());
    }
